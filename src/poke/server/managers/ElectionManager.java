...
//			elb.setHops((totalConns/2)+1);
	...
	private void respondToWhoIsTheLeader(Management mgmt) {
		...
		mhb.setOriginator(conf.getNodeId());
		mhb.setTime(System.currentTimeMillis());
		mhb.setSecurityCode(-999);	// Added by Aditi Rajawat
			...
			synchronized (syncPt) {
				...
				try {
					...
					if (election instanceof FloodMaxElection) {
						logger.warn("Node " + conf.getNodeId() + " setting max hops to arbitrary value (1)");
						((FloodMaxElection) election).setMaxHops(1);
					}
